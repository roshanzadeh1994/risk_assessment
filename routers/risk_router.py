"""
API Router for managing risks in the system.
"""
...
async def run_risk_workflow(risk_id: int, db: Session):
    ...
    task1 = Task(
        ...
        description="Erste Sicherheitsbewertung",
    )
    ...
    await asyncio.sleep(10)  # in local ist in Ordnung
...
def create_risk(
    risk: RiskCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
